...
_MYSQL_USER = 'davasqueza'
_MYSQL_PASSWORD = '7oTYDkbdg1'
...
def do_features_list(database, table):
    ...
    encoding = flask.request.args.get('encoding', default='utf8')
    ...
    if 'error' in feature_collection:
		...
		response = flask.Response(response=geojson.dumps(feature_collection, sort_keys=True, encoding=encoding, default=outputJSON),mimetype='application/json',status=200)
...
def do_pip(database, table):
    ...
    if 'error' in polygon:
		...
		response = flask.Response(response=geojson.dumps(polygon, sort_keys=True , encoding=encoding, default=outputJSON),mimetype='application/json',status=200)
		response.headers.add("Access-Control-Allow-Origin", "*")
		return response
